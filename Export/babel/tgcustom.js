...
function addScriptTG(nameOBJ) {
  ...
  var dataBooking = eBaDataLayer.page_code === 'CONF' || eBaDataLayer.page_code === 'PURC' ? "fbq('track', 'Purchase'," + objDataBooking() + ")" : '';
  ...
  chkEXP: function chkEXP(C, H, E, D, K, P) {
    ...
    if (expBuy == "null") {
      ...
      if (expStart <= this.today && this.today <= expEnd && this.postpone == "NO") {
        ...
        // console.log(expStart,this.today,expBuy)
        console.log(expStart, eBaDateOut, expBuy, expEnd, expBuyStart); // console.log("xx")
...
var startFNJS = function startFNJS() {
  switch (eBaDataLayer.page_code) {
    case "FPOW":
      console.log("eBaDataLayer.page_code = " + eBaDataLayer.page_code); // implibdx.core.updateDom("footer#main-layout-bottom",function(){
...
jQuery(document).on("plnext:customData:ready", startFNJS); // const fnScript = () =>{	
// 	const objStart = {
// 		FDCS:()=>{
// 			(/\b^[DK]{2}/).test(eBaDataLayer.external_id) === true && chkSite() === true? addScriptTG('fbpixel_DK'):console.log('ignore-script')
// 		}
